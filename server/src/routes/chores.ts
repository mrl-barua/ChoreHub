...
router.get('/suggestions', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const history = await prisma.choreHistory.findMany({
      where: { familyId, action: { startsWith: 'completed' } },
      orderBy: { createdAt: 'desc' },
      take: 200,
    });
    const choreIds = Array.from(new Set(history.map((h: any) => h.choreId))) as string[];
    const chores = choreIds.length > 0
      ? await prisma.chore.findMany({ where: { id: { in: choreIds } }, select: { id: true, title: true, category: true } })
      : [];
    const choreMap = new Map(chores.map((c) => [c.id, c]));
    const suggestions: { type: string; message: string; category: string; title?: string }[] = [];
    ...
    history.forEach((h: any) => {
      ...
      const chore = choreMap.get(h.choreId);
      if (!chore) return;
      if (!dayCategory[day]) dayCategory[day] = {};
      dayCategory[day][chore.category] = (dayCategory[day][chore.category] || 0) + 1;
    });
    if (dayCategory[todayDay]) {
      const cats = Object.entries(dayCategory[todayDay]).sort(([, a], [, b]) => (b as number) - (a as number));
      if (cats.length > 0 && (cats[0][1] as number) >= 2) {
        suggestions.push({ type: 'pattern', message: `You usually do ${cats[0][0]} on ${dayNames[todayDay]}s`, category: cats[0][0] });
      }
    }

    // Staleness detection
    const lastDone: Record<string, Date> = {};
    history.forEach((h: any) => {
      const chore = choreMap.get(h.choreId);
      if (!chore) return;
      if (!lastDone[chore.category] || new Date(h.createdAt) > lastDone[chore.category]) {
        lastDone[chore.category] = new Date(h.createdAt);
      }
    });
    const now = new Date();
    Object.entries(lastDone).forEach(([cat, date]) => {
      const days = Math.floor((now.getTime() - date.getTime()) / 86400000);
      if (days >= 5 && suggestions.length < 3) {
        suggestions.push({ type: 'stale', message: `Time for ${cat} — last done ${days} days ago`, category: cat });
      }
    });

    // Most frequent chore not done recently
    const titleCount: Record<string, { count: number; category: string; lastDone: Date }> = {};
    history.forEach((h: any) => {
      const chore = choreMap.get(h.choreId);
      if (!chore) return;
      if (!titleCount[chore.title]) titleCount[chore.title] = { count: 0, category: chore.category, lastDone: new Date(0) };
      titleCount[chore.title].count++;
      if (new Date(h.createdAt) > titleCount[chore.title].lastDone) titleCount[chore.title].lastDone = new Date(h.createdAt);
    });
    for (const [title, data] of Object.entries(titleCount).sort(([, a], [, b]) => b.count - a.count)) {
      const daysSince = Math.floor((now.getTime() - data.lastDone.getTime()) / 86400000);
      if (daysSince >= 3 && suggestions.length < 3) {
        suggestions.push({ type: 'frequent', message: `"${title}" — you do this often`, category: data.category, title });
        break;
      }
    }
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      // Auto-generate next recurring chore
      let nextChoreId = null;
      if (chore.recurrence) {
        const baseDue = chore.dueDate ? new Date(chore.dueDate) : new Date();
        let nextDue = new Date(baseDue);
        switch (chore.recurrence) {
          case 'daily': nextDue.setDate(baseDue.getDate() + 1); break;
          case 'weekly': nextDue.setDate(baseDue.getDate() + 7); break;
          case 'monthly': nextDue.setMonth(baseDue.getMonth() + 1); break;
        }
        const newChore = await prisma.chore.create({
          data: {
            ...
            assignedTo: chore.assignedTo,
            assignmentStatus: chore.assignedTo ? 'pending_acceptance' : 'unassigned',
        ...
        nextChoreId = newChore.id;
        await recordHistory(newChore.id, chore.familyId, req.userId!, 'created');
      }

      res.json({ success: true, nextChoreId });
    } catch (error) {
...
router.get('/:id/comments', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!(await verifyFamilyMembership(req.userId!, chore.familyId))) { res.status(403).json({ error: 'Forbidden' }); return; }

    const comments = await prisma.choreComment.findMany({
      where: { choreId: id },
      orderBy: { createdAt: 'asc' },
    });
    const userIds = Array.from(new Set(comments.map((c: any) => c.userId))) as string[];
    const users = userIds.length > 0 ? await prisma.user.findMany({ where: { id: { in: userIds } }, select: { id: true, displayName: true } }) : [];
...
router.post('/:id/comments', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const comment = await prisma.choreComment.create({
      data: { choreId: id, userId: req.userId!, text: text.trim() },
    });
